...
%% @end
%%--------------------------------------------------------------------
handle_call({parse_file, File, IncludePaths}, _From,
	    State) ->
...
parse_gen_statem(TokenList) ->
    ...
    Fun = fun ({function, _, init, 1, Clauses}, Acc) ->
		  parse_function_add_edges(Clauses, init, [async], Type,
					   Acc);
	      ({function, _, handle_event = FnName, 4, Clauses},
	       Acc) ->
		  parse_function_add_node_and_edges(Clauses, FnName, [],
						    Type, Acc);
	      ({function, _, FnName, 3, Clauses}, Acc) ->
		  parse_function_add_node_and_edges(Clauses, FnName, [],
						    Type, Acc);
	      ({function, _, FnName, _Arity, Clauses}, Acc) ->
		  parse_function_add_node_and_edges(Clauses, FnName, [],
						    Type, Acc);
	      (Other, {OldNodes, OldEdges, OldAllStates}) ->
		  io:format("unhandled_clause:~n ~p~n", [Other]),
		  {OldNodes, OldEdges, OldAllStates}
	  end,
    Graph = generic_parse(TokenList, Fun),
    remove_unreachable(Graph, init),
    {parsed, Type, Graph}.

remove_unreachable(Graph, Start) ->
    [StartV] = [V
		|| V <- digraph:vertices(Graph),
		   {_, Label} <- [digraph:vertex(Graph, V)],
		   Label == Start],
    io:format("StartV = ~p~n", [StartV]),
    lists:foreach(fun (V) ->
			  case V /= StartV andalso
				 digraph:get_path(Graph, StartV, V) == false
			      of
			    true ->
				io:format("removing vertex = ~p~n", [V]),
...
parse_gen_fsm(TokenList) ->
    ...
    Fun = fun ({function, _, init, 1, Clauses}, Acc) ->
		  ...
		  parse_function_add_states(Clauses, handle_event,
					    [async, allstate], Type, Acc);
	      ({function, _, handle_sync_event, 4, Clauses}, Acc) ->
		  parse_function_add_states(Clauses, handle_sync_event,
					    [sync, allstate], Type, Acc);
	      ({function, _, handle_info, 3, Clauses}, Acc) ->
		  parse_function_add_states(Clauses, handle_info,
					    [async, allstate, info], Type, Acc);
...
generic_parse(TokenList, Fun) ->
    Graph = digraph:new(),
    {States, Edges, AllStateEdges} = lists:foldl(Fun,
						 {[init, terminate], [], []},
						 TokenList),
    {ExpAllStates, Expansion} = case
				  application:get_env(erlesy, expand, true)
				    of
				  true -> {States, States};
				  false -> {["*" | States], ["*"]}
				end,
    NewAllStateEdges = expand_allstates(AllStateEdges,
					Expansion), %States),
    ...
    lists:foreach(fun (#edge{vertex1 = From, vertex2 = To,
			     edge_data = Data}) ->
...
parse_gen_event(_TokenList) -> {error, not_supported}.

parse_function_add_states(Clauses, FnName, Options,
			  Type, {OldNodes, OldEdges, OldAllStates}) ->
...
parse_function_add_node_and_edges(Clauses, FnName,
    ...
    case parse_function(Clauses, FnName, Options, Type) of
      {error, _Reason} -> {OldNodes, OldEdges, OldAllStates};
      NewEdges ->
	  {[FnName | OldNodes], OldEdges ++ NewEdges,
	   OldAllStates}
    end.

parse_function(Clauses, FnName, Options, Type) ->
    Edges = lists:foldl(fun (Clause, AccIn) ->
				[parse_function_clause(Clause, FnName, Options,
						       Type)
				 | AccIn]
			end,
			[], Clauses),
    ...
    %       io:format(">>> ~p~n", [E])
    %   end, FlatEdges),
    case lists:all(fun (El) -> {error, bad_transition} == El
		   end,
		   Edges)
	of
      false -> FlatEdges;
...
parse_function_clause({clause, _Line, Args, Guards,
		       Body},
		      init, _Options, Type) ->
    Fun = fun ({ok, NextState, init}) ->
		  PrettyGuards = lists:map(fun (G) -> erl_pp:guard(G) end,
					   Guards),
		  PrettyBody = erl_pp:exprs(Body),
		  PrettyArgs = erl_pp:exprs(Args),
		  #edge{vertex1 = init, vertex2 = NextState,
			edge_data =
			    #edge_data{event = "", args = PrettyArgs,
				       pattern = Args, guard = PrettyGuards,
				       code = PrettyBody, attributes = [async]}}
	  end,
    map_parse_func(Fun, init, Body, Type);
parse_function_clause({clause, _Line, [Event | Args],
		       Guards, Body},
    ...
    Fun = fun ({ok, NextState, RetType}) ->
		  ...
		  PrettyArgs = erl_pp:exprs(Args),
		  #edge{vertex1 = handle_info, vertex2 = NextState,
			edge_data =
			    #edge_data{event = PrettyEvent, args = PrettyArgs,
				       pattern = Args, guard = PrettyGuards,
				       code = PrettyBody,
				       attributes = [RetType | Options]}}
	  end,
    map_parse_func(Fun, handle_info, Body, gen_fsm);
parse_function_clause({clause, _Line,
		       [_EventType, State, Event | Args], Guards, Body},
    ...
    Fun = fun ({ok, NextState, RetType}) ->
		  ...
		  PrettyEvent = erl_pp:expr(Event),
		  PrettyArgs = erl_pp:exprs(Args),
		  #edge{vertex1 = State, vertex2 = NextState,
			edge_data =
			    #edge_data{event = PrettyEvent, args = PrettyArgs,
				       pattern = Args, guard = PrettyGuards,
				       code = PrettyBody,
...
parse_function_clause({clause, _Line, [Event | Args],
    ...
    Fun = fun ({ok, NextState, _}) ->
		  ...
		  PrettyEvent = erl_pp:expr(Event),
		  PrettyArgs = erl_pp:exprs(Args),
		  #edge{vertex1 = FnName, vertex2 = NextState,
			edge_data =
			    #edge_data{event = PrettyEvent, args = PrettyArgs,
				       pattern = Args, guard = PrettyGuards,
...
parse_function_clause({clause, _Line, Args1, Guards,
		       Body},
		      FnName, Options, gen_statem) ->
    {Event, Args} = case Args1 of
		      [{tuple, _, [{atom, _, _}, _]}, E | A] -> {E, A};
		      [{atom, _, _}, E | A] -> {E, A};
		      _ -> {{atom, 0, function_call}, Args1}
		    end,
    Fun = fun ({ok, NextState, _}) ->
		  PrettyGuards = lists:map(fun (G) -> erl_pp:guard(G) end,
					   Guards),
...
parse_body(Body) ->
    lists:flatten(parse_statement(Body)).

parse_statement({tuple, Line, Elems}) ->
    [{tuple, Line, Elems}];
parse_statement({call, Line, Func, Args}) ->
    [{tuple, Line, [{atom, Line, next_state}, Func, Args]}];
parse_statement({match, _Line, _Left, Right}) ->
    parse_statement(Right);
parse_statement({'case', _Line, _Expr, Clauses}) ->
    [parse_statement(C) || C <- Clauses];
parse_statement({clause, _Line, _Pattern, _Guards,
		 Right}) ->
    parse_statement(Right);
parse_statement([X]) -> parse_statement(X);
parse_statement([_X | Xs]) -> parse_statement(Xs);
parse_statement({var, _Line, _}) -> [];
parse_statement({atom, _Line, _}) -> [];
parse_statement({record, _Line, _Name, _Fields}) -> [];
parse_statement({cons, _Line, _X, _Xs}) -> [];
...
parse_statement(Statement) ->
    io:format("parse_statement, unhandled:~n ~p~n",
	      [Statement]),
    [].
...
eval_return(State, [ReturnVal | Rest], gen_fsm, Acc) ->
    case eval_tuple(ReturnVal) of
	  ...
	  eval_return(State, Rest, gen_fsm,
		      [{ok, NextState, init} | Acc]);
      {ok, {ok, NextState, _Data, _Timeout}} ->
	  eval_return(State, Rest, gen_fsm,
		      [{ok, NextState, init} | Acc]);
      {ok, {stop, _Reason}} ->
	  eval_return(State, Rest, gen_fsm,
		      [{ok, terminate, init} | Acc]);
      {ok, {reply, _Reply, NextState, _Data}} ->
	  eval_return(State, Rest, gen_fsm,
		      [{ok, NextState, sync} | Acc]);
      {ok, {reply, _Reply, NextState, _Data, _Timeout}} ->
	  eval_return(State, Rest, gen_fsm,
		      [{ok, NextState, sync} | Acc]);
      {ok, {stop, _Reason, _Reply, _Data}} ->
	  eval_return(State, Rest, gen_fsm,
		      [{ok, terminate, sync} | Acc]);
      {ok, {next_state, NextState, _Data}} ->
	  eval_return(State, Rest, gen_fsm,
		      [{ok, NextState, async} | Acc]);
      {ok, {next_state, NextState, _Data, _Timeout}} ->
	  eval_return(State, Rest, gen_fsm,
		      [{ok, NextState, async} | Acc]);
      {ok, {stop, _Reason, _Data}} ->
	  eval_return(State, Rest, gen_fsm,
		      [{ok, terminate, async} | Acc]);
      Other ->
	  io:format("unhandled case: ~p~n", [Other]),
	  eval_return(State, Rest, gen_fsm, Acc)
    end;
eval_return(State, [ReturnVal | Rest], gen_statem,
	    Acc) ->
    case eval_tuple(ReturnVal) of
      {ok, {ok, NextState, _Data}} ->
	  eval_return(State, Rest, gen_statem,
		      [{ok, NextState, init} | Acc]);
	  ...
	  eval_return(State, Rest, gen_statem,
		      [{ok, State, async} | Acc]);
      _Other -> eval_return(State, Rest, gen_statem, Acc)
    end;
eval_return(_, [], _, []) -> {error, badreturn};
eval_return(_, [], _, Acc) -> Acc.
...
expand_allstates(Edges, States) ->
    ClearStates = lists:delete(init,
			       lists:delete(terminate, States)),
    lists:foldl(fun (Edge, Acc) ->
			#edge{vertex1 = FnName, vertex2 = To,
			      edge_data = Data} =
			    Edge,
			...
			case To of
			  '@var' ->
			      case lists:nth(StateIndex, Data#edge_data.pattern)
				  of
