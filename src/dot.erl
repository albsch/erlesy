...
digraph_to_dot(Name, G) ->
    "digraph \"" ++
      Name ++
	"\" { \n" ++
	  %% These next few elements are added as they look
	  %% a bit nicer in http://www.webgraphviz.com for the test,
	  %% @TODO make these dynamic based on the diagraph data
...
edges_to_dot(G, [E | T]) ->
    ...
    case EdgeLabel#edge_data.guard of
      [] ->
	  R = io_lib:format("~s",
			    [filter_newline(lists:flatten(EdgeLabel#edge_data.event))]);
      Guard ->
	  R = io_lib:format("~s [~s]",
			    [filter_newline(lists:flatten(EdgeLabel#edge_data.event)),
			     Guard])
    ...
    "  " ++
      atom_to_list(VLabel1) ++
	"->" ++
	  atom_to_list(VLabel2) ++
	    "[label=\"" ++ R ++ "\"]" ++ "\n" ++ edges_to_dot(G, T).

% statename(init) -> "[*]";
% statename(terminate) -> "[*]";
% statename(Other) -> atom_to_list(Other).
